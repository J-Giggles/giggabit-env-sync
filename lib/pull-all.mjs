...
import {
  ...
  VERCEL_DEPLOYMENT_TARGET_ORDER,
...
async function fetchMergedProjectVercelMaps(projects) {
  ...
  try {
    for (const project of projects) {
      ...
      try {
        ...
        const perProjectTargets = distinctVercelDeploymentTargets(envs);
        for (const envName of perProjectTargets) {
          ...
          try {
            ...
            const message =
              targetError instanceof Error
                ? targetError.message
                : String(targetError);
            throw new Error(`[${envName}] ${message}`, { cause: targetError });
          }
        }
      } catch (error) {
  ...
  const targets = VERCEL_DEPLOYMENT_TARGET_ORDER.filter((target) =>
