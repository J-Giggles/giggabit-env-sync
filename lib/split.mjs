...
/**
 * Build the filtered and normalized projection written to working `.env*` files.
 * Current rules omit ephemeral/Vercel-injected keys, Turbo/Nx tool noise, Convex
 * keys when disabled, and redundant legacy aliases, then normalize deployment metadata.
 * The implementation below is the canonical rule set as those categories evolve.
 * @param {Map<string, string>} map
 * @returns {Map<string, string>}
 */
export function filterMergedForLocalWorkspace(map) {
  ...
  for (const [alias, canonical] of LEGACY_ALIAS_PAIRS) {
    dropLegacyAliasWhenCanonicalExists(out, alias, canonical);
  }
