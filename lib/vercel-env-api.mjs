...
/**
 * Bearer token for `api.vercel.com` — see `lib/vercel-token.mjs` for resolution order.
 *
 * @returns {string}
 */
...
const ONGOING_UPDATE_RETRY_BASE_MS = 250;
const ONGOING_UPDATE_RETRY_JITTER_MAX_MS = 100;

/**
 * Exponentially back off with a small bounded jitter so concurrent env writers do not
 * remain synchronized.
 *
 * @param {number} attempt zero-based retry attempt
 * @returns {number}
 */
function ongoingUpdateRetryDelayMs(attempt) {
  const jitterMs = Math.floor(
    Math.random() * (ONGOING_UPDATE_RETRY_JITTER_MAX_MS + 1),
  );
  return ONGOING_UPDATE_RETRY_BASE_MS * 2 ** attempt + jitterMs;
}
...
async function fetchWithRetry(url, init, attempt = 0) {
  ...
  try {
    ...
    if (
      ...
      (await isVercelOngoingUpdateResponse(res))
    ) {
      await sleep(ongoingUpdateRetryDelayMs(attempt));
...
/**
 * Vercel returns this 400 while another environment mutation is still being applied.
 * Read a cloned body so the final caller can still consume the original response.
 *
 * @param {Response} response
 * @returns {Promise<boolean>}
 */
async function isVercelOngoingUpdateResponse(response) {
  try {
    const body = await response.clone().json();
    return body?.error?.code === "envs_ongoing_update";
  } catch {
    return false;
  }
}
