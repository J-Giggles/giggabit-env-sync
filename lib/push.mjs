...
function logConvexPushPlan(target, localMap, fromSync, convexFiltered) {
  ...
  if (typeof dk === "string" && dk.trim()) {
    const head = dk.trim().split("|")[0] ?? "";
    syncDim(
      `  CONVEX_DEPLOY_KEY in file: ${head || "(set)"} (value redacted, ${dk.trim().length} chars)`,
    );
  }
...
async function pushVercelMap(map, envName, _opts = {}) {
  ...
  if (!listRes.ok) {
    if (listRes.status === 0) {
      // CLI OAuth login works for `vercel env pull/add` but often has no auth.json token
      // for the REST API. Fall back so agents/local pushes still work.
      syncWarn(
        "Vercel API token missing — falling back to `vercel env` CLI (same auth as env pull).",
      );
      await pushVercelMapViaCli(entries, envName, previewBranch);
      return;
    }
    throw new Error(
      `Vercel API list failed (${listRes.status}): ${listRes.body.slice(0, 500)}`
    );
  }
...
/**
 * Upsert via `vercel env update` / `add --force`, piping values on stdin (never argv).
 *
 * @param {Array<[string, string]>} entries
 * @param {"development" | "preview" | "production"} envName
 * @param {string} previewBranch
 */
async function pushVercelMapViaCli(entries, envName, previewBranch) {
  syncInfo(`Vercel CLI upsert: ${entries.length} key(s) → ${envName}…`);
  /** @type {string[]} */
  const failures = [];
  for (const [key, value] of entries) {
    ...
    if (envName === "preview" && previewBranch) {
      updateArgs.push(previewBranch);
    }
    let r = runVercel(updateArgs, { input: value });
    if (!r.ok) {
      /** @type {string[]} */
      const addArgs = [
        "env",
      ...
      if (envName === "preview" && previewBranch) {
        addArgs.push(previewBranch);
      }
      r = runVercel(addArgs, { input: value });
    }
    if (!r.ok) {
      failures.push(key);
      const err = (r.stderr || r.stdout || "").trim().slice(0, 160);
      syncWarn(`  • ${key} failed: ${err || `exit ${r.status}`}`);
    }
  }
  if (failures.length > 0) {
    throw new Error(
      `Vercel CLI upsert failed for ${failures.length} key(s): ${failures.slice(0, 8).join(", ")}${
        failures.length > 8 ? "…" : ""
      }`,
    );
  }
