...
import { printPushPlan } from "./print-push-plan.mjs";
import { printPushVerification } from "./print-push-verification.mjs";
import {
 ...
 *   `approved`: skip drift/local confirmations (batch preview already approved).
 *   `skipPlanPrint`: do not print the per-target plan table (caller printed batch preview).
 *   `plan`: reuse a plan from {@link planPushTarget} (avoids re-fetching remote).
 *   `skipVerification`: do not print post-push verification (batch callers may verify once).
 */
export async function pushTarget(target, opts = {}) {
  const skipConfirm = opts.yes === true || opts.approved === true;
  ...
  const plan =
    opts.plan ??
    (await (async () => {
      syncInfo(`Planning push for "${target}"…`);
      return planPushTarget(target, {
  ...
  if (!opts.skipPlanPrint) {
    printPushPlan(plan);
  }

  syncInfo(`Source file: ${plan.localRel}`);
  if (plan.fromSync) {
    syncInfo(
      `--from-sync: Convex uses ${plan.convexKind} deployment (CONVEX_DEPLOY_KEY / URLs in sync file).`
    );
  }
  ...
  if (plan.localMap.get("CONVEX_DEPLOY_KEY")?.trim()) {
    syncInfo(
  ...
  if (plan.localChangedSinceLastPush) {
    syncWarn(
  ...
  if (target === "preview" && !plan.fromSync) {
    syncWarn(
  ...
  for (const key of plan.convexDroppedLocalhost) {
    syncWarn(
      `Skipping ${key}=${JSON.stringify(plan.localMap.get(key))} for Convex push — Convex cloud cannot reach localhost. ` +
  ...
  if (!plan.force && plan.convexSkipped > 0) {
    syncInfo(
      `Convex diff: ${plan.convexToPush.size} key(s) to push, ${plan.convexSkipped} unchanged → skipped.`
    );
  }

  const convexToPush = plan.convexToPush;
  if (convexToPush.size > 0) {
    ...
    const cArgs = [
      ...
      convexFile,
      "--force",
      ...convexEnvSetSuffixArgs(target, plan.localMap, plan.fromSync),
    ];
    syncInfo(
      `Running convex env set --from-file (${target} sync target, ${convexToPush.size} key(s))…`
    );
    const cRes = pnpmExec("convex", cArgs, { env: plan.convexCliEnv });
    try {
  ...
  if (!convexOnly && plan.vercelToPush && plan.vercelEnv) {
    const vEnv = plan.vercelEnv;
    const vercelToPush = plan.vercelToPush;
    syncInfo(
      plan.force
        ? `Pushing to Vercel (${vEnv}) — ${vercelToPush.size} key(s) (--force)…`
        : `Pushing to Vercel (${vEnv}) — ${vercelToPush.size} changed/new, ${plan.vercelSkipped} unchanged skipped.`
    );
  ...
  const afterConvex = hashEnvMap(
    plan.fromSync
      ? fetchConvexEnvMapOptions({
          useProd: inferConvexUseProdFromLocalMap(plan.localMap, target),
          convexEnv: plan.convexCliEnv,
        })
      : fetchConvexEnvMap(target, plan.convexCliEnv)
  );
  if (convexOnly) {
    patchTarget(target, {
      convexHash: afterConvex,
      lastPushedLocalHash: plan.localSourceHash,
      lastPushAt: now,
    });
  } else {
...
/**
 * Plan pushes for multiple targets (fetches remote for each).
 *
 * @param {TTarget[]} targets
 * @param {{ fromSync?: boolean; convexOnly?: boolean; force?: boolean }} [opts]
 * @returns {Promise<import("./push-plan.mjs").TPushPlan[]>}
 */
export async function planPushTargets(targets, opts = {}) {
  /** @type {import("./push-plan.mjs").TPushPlan[]} */
  const plans = [];
  for (const t of targets) {
    syncInfo(`Planning push for "${t}"…`);
    plans.push(
      await planPushTarget(t, {
        fromSync: opts.fromSync === true,
        convexOnly: opts.convexOnly === true,
        force: opts.force === true,
      })
    );
  }
