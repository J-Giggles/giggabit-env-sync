...
import { filterLocalMissingOnRemoteNonEmpty } from "./env-map-filters.mjs";
 ...
 * @param {{ yes?: boolean; fromSync?: boolean; vercelSensitive?: "default" | "on" | "off"; convexOnly?: boolean; force?: boolean; missingOnly?: boolean; metadataNamespace?: string }} [opts]
 ...
 *   `missingOnly`: push only non-empty local keys that are absent on the remote host (no overwrites).
 */
export async function pushTarget(target, opts = {}) {
  ...
  if (missingOnly) {
    syncInfo(
      "Missing-only: will push non-empty local keys that are absent on Convex/Vercel (existing remote values are not updated)."
    );
    if (force) {
      syncWarn("--force is ignored with --missing-only.");
    }
  }
  if (!convexEnabled) {
  ...
  if (convexEnabled) {
     ...
     */
    const convexToPush = missingOnly
      ? filterLocalMissingOnRemoteNonEmpty(convexFiltered, remoteConvex)
      : force
        ? convexFiltered
    ...
    if (missingOnly) {
      syncInfo(
        `Convex missing-only: ${convexToPush.size} new key(s) to push, ${convexSkipped} skipped (already on host or empty locally).`
      );
    } else if (!force && convexSkipped > 0) {
      syncInfo(
  ...
  if (!convexOnly) {
    ...
    const vercelToPush = missingOnly
      ? filterLocalMissingOnRemoteNonEmpty(
          vercelFiltered,
          remoteVercel ?? new Map()
        )
      : force
        ? vercelFiltered
    ...
    syncInfo(
      missingOnly
        ? `Vercel (${vEnv}) missing-only: ${vercelToPush.size} new key(s), ${vercelSkipped} skipped.`
        : force
