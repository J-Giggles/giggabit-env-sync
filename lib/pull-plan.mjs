...
import { printSection, printTable, syncInfo, syncWarn } from "./cli-output.mjs";
...
/**
 * @param {{ missingOnly?: boolean; convexDevMap?: Map<string, string>; convexProdMap?: Map<string, string>; vercelMaps?: Map<string, Map<string, string>> }} [opts]
...
export async function computePullAllPlans(opts = {}) {
  ...
  if (opts.vercelMaps) {
    ...
    const { envs } = await fetchVercelProjectEnvList();
...
export function printPullPreviewPlans(plans, opts) {
  ...
  printNote(
    `Total: ${total} key(s) across ${plans.length} file(s) · Vercel wins on conflict with Convex`
  );
  ...
  for (const plan of plans) {
    ...
    printTable(
      ["KEY", "SOURCE", "VALUE"],
      shown.map((row) => [row.key, row.source, row.valuePreview]),
      { compact: true }
    );
