...
function runVercelInherited(args) {
  const cwd = getVercelProjectCwd();
  try {
    runInherited("vercel", args, { cwd });
  } catch (err) {
    ...
    runInherited("pnpm", ["dlx", "vercel", ...args], { cwd });
  }
}

/**
 * Arguments for deploying Convex through the configured package. In monorepo mode this
 * runs from the backend workspace; in single-repo mode it preserves `pnpm exec convex`.
 *
 * @returns {string[]}
 */
function buildConvexDeployCommandArgs() {
  const pkg = getConvexPackageFilter();
  if (pkg) return ["--filter", pkg, "exec", "convex", "deploy"];
  return ["exec", "convex", "deploy"];
}
...
export async function deployTarget(args) {
  ...
  if (convexEnabled && !args.skipConvexDeploy) {
    ...
    runInherited("pnpm", buildConvexDeployCommandArgs(), { env: convexEnv });
  }
