 ...
 *   confirmBeforeWrite?: (changeCount: number) => Promise<boolean>;
 * }} opts
 */
export async function interactivePull(opts = {}) {
  ...
  const rl = opts.rl;
  const confirmBeforeWrite = opts.confirmBeforeWrite;
  ...
  if (idx === -1) {
    ...
    const go = confirmBeforeWrite
      ? await confirmBeforeWrite(total)
      : true;
    if (!go) {
  ...
  syncInfo("Fetching hosted env and building pull preview…");
  let plan = planPullFromExecuteOpts(executeOpts, null);
  printPullPlans([plan]);
  if (rl) {
    const refined = await refinePlansWithKeyFilterAfterPreview(rl, "pull", [plan], {
      printPlans: (p, o) => printPullPlans(p, o),
    });
    plan = refined.plans[0] ?? plan;
  }
  const total = countPullPlanChanges([plan]);
  if (total === 0) {
