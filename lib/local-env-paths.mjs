/**
 * Which active Vercel project `.env*` files correspond to each sync target (same rules as push source).
 */
...
export function resolveLocalEnvReadPath(target) {
  ...
  for (const rel of localEnvCandidates(target)) {
    const p = path.join(projectCwd, rel);
...
/**
 * Uncommon Convex/Vercel pairings write **`.env.sync.<storageKey>`** so project `.env*.local`
 ...
 * Paths are relative to the active Vercel project cwd; in single-repo mode that is `REPO_ROOT`.
 *
 * @param {import("./remote.mjs").TTarget} target
 * @returns {{ abs: string, rel: string, created: boolean }}
 */
export function resolveLocalEnvWritePath(target) {
  ...
  for (const rel of rels) {
    ...
    if (fs.existsSync(abs)) {
      return { abs, rel: path.relative(REPO_ROOT, abs), created: false };
    }
  }
  const rel = rels[0];
  const abs = path.join(projectCwd, rel);
  return { abs, rel: path.relative(REPO_ROOT, abs), created: true };
}
