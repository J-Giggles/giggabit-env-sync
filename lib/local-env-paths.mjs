 ...
 * Returns `null` for uncommon pairings. `resolveOddPairPullPath` only resolves the root
 * `.env.sync.<storageKey>` artifact path; callers write it, so root `.env*.local` files
 * stay untouched.
 *
...
/**
 * Resolve the root **`.env.sync.<storageKey>`** artifact path for uncommon Convex/Vercel
 * pairings. The caller is responsible for writing it; root `.env*.local` files are not
