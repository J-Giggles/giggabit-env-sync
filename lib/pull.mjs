...
import {
  countKeysMissingFromLocal,
  mergeRemoteKeysMissingIntoLocal,
} from "./env-map-filters.mjs";
...
export async function executePull(opts) {
  ...
  if (!snapshotOnly && localWrite) {
    ...
    const existingMap = fs.existsSync(localAbs)
      ? parseDotenv(fs.readFileSync(localAbs, "utf8"))
      : new Map();
    /** @type {Map<string, string>} */
    let forLocal = remoteForLocal;
    if (opts.missingOnly) {
      const addCount = countKeysMissingFromLocal(existingMap, remoteForLocal);
      forLocal = mergeRemoteKeysMissingIntoLocal(existingMap, remoteForLocal);
      syncInfo(
        `Missing-only: kept ${existingMap.size} existing key(s) in ${localRel}, added ${addCount} from host.`
      );
    }
