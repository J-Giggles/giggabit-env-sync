...
export async function executePull(opts) {
  ...
  if (templateAbs) {
    try {
      templateContent = fs.readFileSync(templateAbs, "utf8");
    } catch (e) {
      syncWarn(
  ...
  const mergedBody = buildMergedSnapshotBody(merged, templateContent);
  ...
  if (!snapshotOnly && localWrite) {
    ...
    /** @type {string} */
    let localBody;
    if (templateContent && templateAbs) {
 ...
 * @param {string | null | undefined} templateContent
 */
export function buildMergedSnapshotBody(merged, templateContent) {
  return templateContent
    ? formatEnvFromExampleTemplate(templateContent, merged)
    : serializeDotenv(merged);
}
