...
export async function executePull(opts) {
  ...
  if (opts.keyFilter) {
    ...
    for (const k of merged.keys()) {
      if (keyMatchesFilter(k, opts.keyFilter)) n++;
    }
    mergedForWrite = applyKeyFilterToMap(prevMap, merged, opts.keyFilter);
    syncInfo(
      `Key filter "${opts.keyFilter.raw}": updating ${n} key(s) in snapshot (others preserved).`
    );
  }
  const mergedBody = serializeDotenv(mergedForWrite);
  ...
  if (!snapshotOnly && localWrite) {
    ...
    let forLocal;
    if (opts.keyFilter) {
      const existingLocal = fs.existsSync(localAbs)
        ? parseDotenv(fs.readFileSync(localAbs, "utf8"))
        : new Map();
      forLocal = applyKeyFilterToMap(existingLocal, mergedLocal, opts.keyFilter);
    } else {
      forLocal = filterMergedForLocalWorkspace(mergedForWrite);
    }
  ...
  if (!prevExists) {
    ...
    const changes = diffMaps(prevMap, mergedForWrite);
    syncInfo(
 ...
 * @param {Map<string, string>} vercelMap
 * @param {Map<string, string>} convexMap
 * @param {{ quiet?: boolean }} [opts] — one summary line instead of per-key warnings
 */
export function mergeWithWarnings(vercelMap, convexMap, opts = {}) {
  ...
  if (quiet && differing.length > 0) {
    const sample = differing.slice(0, 6).join(", ");
    const more =
      differing.length > 6 ? ` (+${differing.length - 6} more)` : "";
    syncWarn(
      `${differing.length} key(s) differed between Vercel and Convex — snapshot uses Vercel: ${sample}${more}`
    );
  }
  return out;
}
