...
/**
 * Pin Vercel CLI calls to the linked project and configured repo team.
 *
...
export function buildVercelCliEnv(base = process.env) {
  ...
  const teamId = process.env.ENV_SYNC_VERCEL_TEAM_ID?.trim();
 ...
 *
 * @param {string[]} args
 * @param {{ cwd?: string; env?: NodeJS.ProcessEnv }} [opts] — override cwd/env.
 */
export function runVercel(args, opts = {}) {
  ...
  let r = run("vercel", args, { cwd, env });
  if (!r.ok && (r.error || /not found|ENOENT/i.test(r.stderr))) {
    r = run("pnpm", ["dlx", "vercel", ...args], { cwd, env });
  }
  return r;
}
