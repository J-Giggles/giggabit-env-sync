 ...
 * - **Cache directory** (`cachePath`) — `.env/sync/`, or `.env.sync-cache/` when the
 *   root `.env` path is a file. Contains ephemeral Vercel pull caches and Convex `env set`
 *   temp files (not meant for editing).
 */
...
/**
 * Files under {@link SYNC_DIR} only — caches and short-lived Convex push payloads.
