...
export async function interactivePushCli() {
  ...
  try {
    ...
    const missingOnly = await askYesNo(
      rl,
      "Missing keys only? Push non-empty local vars absent on host (no overwrites)"
    );

    syncInfo(
      `Summary: targets=${targets.join(",")} · fromSync=${fromSync} · vercelSensitive=${vercelSensitive} · skipConfirm=${yes} · missingOnly=${missingOnly}`
    );
    ...
    /** @type {{ yes: boolean; fromSync: boolean; vercelSensitive: "default" | "on" | "off"; missingOnly: boolean }} */
    const pushOpts = {
      yes,
      fromSync,
      vercelSensitive,
