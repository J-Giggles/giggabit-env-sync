 ...
 *
 * - **`ENV_SYNC_CONVEX_PACKAGE=<workspace-name>`** — optional pnpm workspace package filter
 *   used for Convex CLI calls (e.g. `@giggabit/backend`). When unset, Convex CLI calls run
 *   from the repo root, preserving the original single-repo flow.
 *
 * All toggles are backwards-compatible: unset values reproduce the original behavior.
 */
...
/**
 * Optional pnpm workspace package filter for Convex CLI calls. Defaults to `null` so
 * single-package repos continue to run `pnpm exec convex ...` from the repo root.
 *
 * @returns {string | null}
 */
export function getConvexPackageFilter() {
  const raw = process.env.ENV_SYNC_CONVEX_PACKAGE?.trim() ?? "";
  return raw || null;
}
