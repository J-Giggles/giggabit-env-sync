/**
 * env:sync:check — compare a local env file with hosted Convex + Vercel env.
 *
 * Reads the matching `.env.sync.<env>` snapshot by default (same source as `env:sync:push`),
 * applies the Convex/Vercel split rules, fetches both remotes, and prints a diff table per
 * platform. Exits 0 when both platforms match the local view, 1 otherwise. Quiet mode
 * (`-q` / `--quiet`) prints only `true` / `false`.
 *
 * No `convex env set` / `vercel env` writes are performed — read-only by design.
 */
import fs from "node:fs";
import path from "node:path";
import { isConvexEnabled } from "./config.mjs";
import {
  buildConvexCliEnvForPushLocalMap,
  inferConvexUseProdFromLocalMap,
} from "./exec.mjs";
import {
  loadEnvExampleKeys,
  loadEnvTsKeys,
  resolveEnvTsPath,
  resolveEnvTsPaths,
} from "./env-ts-keys.mjs";
import { resolveLocalEnvReadPathForPush } from "./local-env-paths.mjs";
import { parseDotenv } from "./parse-dotenv.mjs";
import { REPO_ROOT } from "./paths.mjs";
import {
  fetchConvexEnvMap,
  fetchConvexEnvMapOptions,
  fetchVercelEnvMap,
  vercelEnvName,
} from "./remote.mjs";
import {
  filterForConvex,
  filterForVercel,
  filterOutVercelAutoInjected,
  isVercelAutoInjectedKey,
} from "./split.mjs";
import {
  syncDim,
  syncInfo,
  syncSuccess,
  syncWarn,
} from "./cli-style.mjs";

/** @typedef {import("./remote.mjs").TTarget} TTarget */

/**
 * Diff two env maps treating empty-string values the same as missing keys
 * (so `KEY=` locally with no remote entry is not flagged as drift).
 *
 * @param {Map<string, string>} local
 * @param {Map<string, string>} remote
 * @returns {{ localOnly: string[]; remoteOnly: string[]; differs: string[] }}
 */
function diffMaps(local, remote) {
  const localOnly = [];
  const remoteOnly = [];
  const differs = [];
  const all = new Set([...local.keys(), ...remote.keys()]);
  for (const k of all) {
    const lv = local.get(k);
    const rv = remote.get(k);
    const lEmpty = lv === undefined || lv === "";
    const rEmpty = rv === undefined || rv === "";
    if (lEmpty && rEmpty) continue;
    if (lEmpty) remoteOnly.push(k);
    else if (rEmpty) localOnly.push(k);
    else if (lv !== rv) differs.push(k);
  }
  localOnly.sort();
  remoteOnly.sort();
  differs.sort();
  return { localOnly, remoteOnly, differs };
}

/**
 * @param {{ localOnly: string[]; remoteOnly: string[]; differs: string[] }} diff
 */
function diffEmpty(diff) {
  return (
    diff.localOnly.length === 0 &&
    diff.remoteOnly.length === 0 &&
    diff.differs.length === 0
  );
}

const ANSI = {
  reset: "\x1b[0m",
  dim: "\x1b[2m",
  bold: "\x1b[1m",
  red: "\x1b[31m",
  green: "\x1b[32m",
  yellow: "\x1b[33m",
  cyan: "\x1b[36m",
};

const VALUE_TRUNCATE = 32;

/**
 * Truncate a value for display (keeps both ends so version-like differences stay visible).
 *
 * @param {string | undefined} value
 */
function fmtValue(value) {
  if (value === undefined) return `${ANSI.dim}—${ANSI.reset}`;
  if (value === "") return `${ANSI.dim}""${ANSI.reset}`;
  if (value.length <= VALUE_TRUNCATE) return value;
  const head = Math.floor((VALUE_TRUNCATE - 1) / 2);
  const tail = VALUE_TRUNCATE - 1 - head;
  return `${value.slice(0, head)}…${value.slice(-tail)}`;
}

/** Width of a string ignoring ANSI escapes (so column padding stays aligned). */
function visibleWidth(s) {
  return s.replace(/\x1b\[[0-9;]*m/g, "").length;
}

/**
 * @param {string} cell
 * @param {number} width
 */
function pad(cell, width) {
  const diff = width - visibleWidth(cell);
  if (diff <= 0) return cell;
  return cell + " ".repeat(diff);
}

const STATUS_LABELS = {
  differs: `${ANSI.yellow}differs${ANSI.reset}`,
  "local-only": `${ANSI.green}local-only${ANSI.reset}`,
  "remote-only": `${ANSI.red}remote-only${ANSI.reset}`,
};

const STATUS_RAW_WIDTH = "remote-only".length;

/**
 * @param {string} title
 * @param {Map<string, string>} local
 * @param {Map<string, string>} remote
 * @param {{ localOnly: string[]; remoteOnly: string[]; differs: string[] }} diff
 */
function printDiffTable(title, local, remote, diff) {
  const inSync = diffEmpty(diff);
  const header = `${ANSI.bold}${title}${ANSI.reset} ${ANSI.dim}· ${local.size} local · ${remote.size} remote${ANSI.reset}`;
  console.log("");
  console.log(header);
  if (inSync) {
    console.log(`  ${ANSI.green}✓ in sync${ANSI.reset}`);
    return;
  }
  console.log(
    `  ${ANSI.yellow}${diff.differs.length}${ANSI.reset} differs · ` +
      `${ANSI.green}${diff.localOnly.length}${ANSI.reset} local-only · ` +
      `${ANSI.red}${diff.remoteOnly.length}${ANSI.reset} remote-only`
  );

  /** @type {Array<{ status: keyof typeof STATUS_LABELS; key: string; localValue: string | undefined; remoteValue: string | undefined }>} */
  const rows = [];
  for (const k of diff.differs) {
    rows.push({
      status: "differs",
      key: k,
      localValue: local.get(k),
      remoteValue: remote.get(k),
    });
  }
  for (const k of diff.localOnly) {
    rows.push({
      status: "local-only",
      key: k,
      localValue: local.get(k),
      remoteValue: undefined,
    });
  }
  for (const k of diff.remoteOnly) {
    rows.push({
      status: "remote-only",
      key: k,
      localValue: undefined,
      remoteValue: remote.get(k),
    });
  }

  const keyWidth = Math.max(
    "Key".length,
    ...rows.map((r) => r.key.length)
  );
  const localWidth = Math.max(
    "Local".length,
    ...rows.map((r) => visibleWidth(fmtValue(r.localValue)))
  );
  const remoteWidth = Math.max(
    "Remote".length,
    ...rows.map((r) => visibleWidth(fmtValue(r.remoteValue)))
  );

  const headerRow =
    "  " +
    pad(`${ANSI.dim}Status${ANSI.reset}`, STATUS_RAW_WIDTH) +
    "  " +
    pad(`${ANSI.dim}Key${ANSI.reset}`, keyWidth) +
    "  " +
    pad(`${ANSI.dim}Local${ANSI.reset}`, localWidth) +
    "  " +
    pad(`${ANSI.dim}Remote${ANSI.reset}`, remoteWidth);
  console.log(headerRow);

  for (const r of rows) {
    const status = STATUS_LABELS[r.status];
    const line =
      "  " +
      pad(status, STATUS_RAW_WIDTH) +
      "  " +
      pad(r.key, keyWidth) +
      "  " +
      pad(fmtValue(r.localValue), localWidth) +
      "  " +
      pad(fmtValue(r.remoteValue), remoteWidth);
    console.log(line);
  }
}

/**
 * @param {TTarget} target
 * @param {{
 *   fromSync?: boolean;
 *   quiet?: boolean;
 *   convexOnly?: boolean;
 *   vercelOnly?: boolean;
 * }} [opts]
 */
export async function checkTarget(target, opts = {}) {
  const fromSync = opts.fromSync !== false;
  const quiet = opts.quiet === true;
  const convexEnabled = isConvexEnabled();
  /** Convex disabled: collapse mode to vercel-only regardless of CLI flags. */
  const convexOnly = opts.convexOnly === true && convexEnabled;
  const vercelOnly = opts.vercelOnly === true || !convexEnabled;
  if (opts.convexOnly && opts.vercelOnly) {
    throw new Error(
      "[env:sync] check: --convex-only and --vercel-only are mutually exclusive."
    );
  }
  if (opts.convexOnly && !convexEnabled) {
    throw new Error(
      "[env:sync] check: --convex-only requested but Convex is disabled (ENV_SYNC_DISABLE_CONVEX=1)."
    );
  }

  const localPath = resolveLocalEnvReadPathForPush(target, { fromSync });
  if (!localPath) {
    throw new Error(
      `[env:sync] check ${target}: no local env file found.${
        fromSync
          ? " Run `pnpm run env:sync:pull -- " + target + "` first."
          : ""
      }`
    );
  }
  const raw = fs.readFileSync(localPath, "utf8");
  const localMap = parseDotenv(raw);

  if (!quiet) {
    syncInfo(
      `Check ${target} — reading ${path.relative(REPO_ROOT, localPath)}`
    );
    if (!convexEnabled) {
      syncDim("  Convex disabled (ENV_SYNC_DISABLE_CONVEX=1) — comparing Vercel only.");
    } else if (fromSync) {
      const kind = inferConvexUseProdFromLocalMap(localMap, target)
        ? "production"
        : "development";
      syncDim(
        `  Convex compare uses ${kind} deployment (CONVEX_DEPLOY_KEY prefix and/or sync target).`
      );
    }
    syncInfo(
      vercelOnly
        ? "Fetching Vercel env…"
        : convexOnly
          ? "Fetching Convex env…"
          : "Fetching Convex + Vercel env…"
    );
  }

  const convexCliEnv = convexEnabled
    ? buildConvexCliEnvForPushLocalMap(localMap, fromSync, target)
    : null;
  const remoteConvex = vercelOnly
    ? null
    : fromSync
      ? fetchConvexEnvMapOptions({
          useProd: inferConvexUseProdFromLocalMap(localMap, target),
          convexEnv: convexCliEnv ?? undefined,
        })
      : fetchConvexEnvMap(target, convexCliEnv ?? undefined);
  const remoteVercel = convexOnly ? null : fetchVercelEnvMap(target);

  const localConvex = filterForConvex(localMap);
  /** Both sides are filtered for Vercel-injected system keys so the diff stays actionable. */
  const localVercel = filterOutVercelAutoInjected(filterForVercel(localMap));
  const remoteVercelUserManaged = remoteVercel
    ? filterOutVercelAutoInjected(remoteVercel)
    : null;

  const convexDiff = remoteConvex
    ? diffMaps(localConvex, remoteConvex)
    : null;
  const vercelDiff = remoteVercelUserManaged
    ? diffMaps(localVercel, remoteVercelUserManaged)
    : null;

  const convexOk = convexDiff === null || diffEmpty(convexDiff);
  const vercelOk = vercelDiff === null || diffEmpty(vercelDiff);
  const inSync = convexOk && vercelOk;

  if (quiet) {
    console.log(inSync ? "true" : "false");
    process.exitCode = inSync ? 0 : 1;
    return;
  }

  if (convexDiff && remoteConvex) {
    printDiffTable(
      `Convex (${inferConvexUseProdFromLocalMap(localMap, target) ? "production" : "development"} deployment)`,
      localConvex,
      remoteConvex,
      convexDiff
    );
  }
  if (vercelDiff && remoteVercelUserManaged) {
    printDiffTable(
      `Vercel ${vercelEnvName(target)}`,
      localVercel,
      remoteVercelUserManaged,
      vercelDiff
    );
  }

  const schemaAudit = auditAgainstEnvTs(
    localMap,
    remoteConvex,
    remoteVercelUserManaged
  );
  if (schemaAudit) {
    printSchemaAudit(schemaAudit);
  }

  console.log("");
  if (inSync) {
    syncSuccess(`✓ In sync — ${target}`);
  } else {
    /** @type {string[]} */
    const reasons = [];
    if (convexDiff && !diffEmpty(convexDiff)) {
      reasons.push(
        `Convex (${convexDiff.differs.length} differ, ${convexDiff.localOnly.length} local-only, ${convexDiff.remoteOnly.length} remote-only)`
      );
    }
    if (vercelDiff && !diffEmpty(vercelDiff)) {
      reasons.push(
        `Vercel (${vercelDiff.differs.length} differ, ${vercelDiff.localOnly.length} local-only, ${vercelDiff.remoteOnly.length} remote-only)`
      );
    }
    syncWarn(`✗ Out of sync — ${reasons.join("; ")}`);
  }

  process.exitCode = inSync ? 0 : 1;
}

/**
 * Infrastructure / build-system keys that env.ts is not expected to validate.
 * Keep narrow — anything project-specific should be in env.ts.
 */
const ENV_TS_AUDIT_IGNORE = new Set([
  "NODE_ENV",
  "CONVEX_DEPLOY_KEY",
  "CONVEX_DEPLOYMENT",
  "CONVEX_URL",
  "VERCEL_OIDC_TOKEN",
]);

/**
 * Keys that belong in `.env.example` / hosted env but are intentionally outside
 * zod `env.ts` (tooling, platform injects). Playwright uses the `E2E_` prefix.
 *
 * @param {string} key
 */
function isEnvTsAuditIgnored(key) {
  if (ENV_TS_AUDIT_IGNORE.has(key)) return true;
  if (isVercelAutoInjectedKey(key)) return true;
  if (key.startsWith("E2E_")) return true;
  return false;
}

/**
 * Validated in env.ts but intentionally omitted from the lean root `.env.example`.
 * Full catalog lives in `docs/setup/ENVIRONMENT_VARIABLES.md` / `docs/testing/e2e.md`.
 */
const ENV_EXAMPLE_OPTIONAL_OMIT = new Set([
  "AGENT_CHAT_MODEL",
  "APP_BASE_URL",
  "NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY",
  "RECEIPT_PORTAL_E2E_ENABLED",
  "STRIPE_CONVEX_RECORDS_METER_EVENT_NAME",
  "STRIPE_STORAGE_USAGE_METER_EVENT_NAME",
  "STRIPE_WEBHOOK_ENDPOINT_URL",
  "TRACKING_BASE_URL",
  "WORKOS_DEV_EMAIL_FALLBACK_ALLOWED",
  "WORKSPACE_PREPARATION_GATE_DISABLED",
]);

const ENV_EXAMPLE_OPTIONAL_OMIT_PREFIXES = [
  /^BROWSERBASE_/u,
  /^DEV_ADMIN_AUTH_/u,
  /^LOCAL_PRIVACY_LOCK_/u,
  /^RECEIPT_PORTAL_/u,
  /^TRUELAYER_/u,
  /^YAPILY_/u,
];

/**
 * @param {string} key
 */
function isOptionalExampleOmit(key) {
  if (ENV_EXAMPLE_OPTIONAL_OMIT.has(key)) return true;
  return ENV_EXAMPLE_OPTIONAL_OMIT_PREFIXES.some((re) => re.test(key));
}

/**
 * Cross-check observed keys (local file + hosted Convex/Vercel) against the
 * project's `env.ts` zod schemas + `.env.example` template.
 *
 * @param {Map<string, string>} localMap
 * @param {Map<string, string> | null} remoteConvex
 * @param {Map<string, string> | null} remoteVercel
 * @returns {{
 *   envTsPath: string;
 *   notInEnvTs: { key: string; sources: string[] }[];
 *   missingFromExample: string[];
 *   missingFromEnvTs: string[];
 *   exampleAvailable: boolean;
 * } | null}
 */
function auditAgainstEnvTs(localMap, remoteConvex, remoteVercel) {
  const envTsKeys = loadEnvTsKeys();
  if (!envTsKeys) return null;
  const envTsPaths = resolveEnvTsPaths();
  const envTsPath = envTsPaths[0] ?? resolveEnvTsPath();
  const exampleKeys = loadEnvExampleKeys();

  /** @type {Map<string, Set<string>>} */
  const observedSources = new Map();
  const recordSource = (key, source) => {
    if (isEnvTsAuditIgnored(key)) return;
    if (!observedSources.has(key)) observedSources.set(key, new Set());
    observedSources.get(key).add(source);
  };
  for (const [k, v] of localMap) {
    if (v !== "") recordSource(k, "local");
  }
  if (remoteConvex) {
    for (const [k, v] of remoteConvex) {
      if (v !== "") recordSource(k, "convex");
    }
  }
  if (remoteVercel) {
    for (const [k, v] of remoteVercel) {
      if (v !== "") recordSource(k, "vercel");
    }
  }

  /** @type {{ key: string; sources: string[] }[]} */
  const notInEnvTs = [];
  for (const [key, sources] of observedSources) {
    if (!envTsKeys.has(key)) {
      notInEnvTs.push({ key, sources: [...sources].sort() });
    }
  }
  notInEnvTs.sort((a, b) => a.key.localeCompare(b.key));

  /** @type {string[]} */
  const missingFromExample = [];
  /** @type {string[]} */
  const missingFromEnvTs = [];
  if (exampleKeys) {
    for (const k of envTsKeys) {
      if (!exampleKeys.has(k) && !isOptionalExampleOmit(k) && !isEnvTsAuditIgnored(k)) {
        missingFromExample.push(k);
      }
    }
    for (const k of exampleKeys) {
      if (!envTsKeys.has(k) && !isEnvTsAuditIgnored(k)) missingFromEnvTs.push(k);
    }
    missingFromExample.sort();
    missingFromEnvTs.sort();
  }

  return {
    envTsPath: envTsPath ?? "env.ts",
    envTsPaths,
    notInEnvTs,
    missingFromExample,
    missingFromEnvTs,
    exampleAvailable: exampleKeys !== null,
  };
}

/**
 * @param {NonNullable<ReturnType<typeof auditAgainstEnvTs>>} audit
 */
function printSchemaAudit(audit) {
  const totalIssues =
    audit.notInEnvTs.length +
    audit.missingFromExample.length +
    audit.missingFromEnvTs.length;
  if (totalIssues === 0) return;

  console.log("");
  const auditPaths =
    audit.envTsPaths?.length > 0
      ? audit.envTsPaths.map((p) => path.relative(REPO_ROOT, p)).join(", ")
      : path.relative(REPO_ROOT, audit.envTsPath);
  console.log(
    `${ANSI.bold}Schema audit${ANSI.reset} ${ANSI.dim}· vs ${auditPaths}${ANSI.reset}`
  );

  if (audit.notInEnvTs.length > 0) {
    console.log(
      `  ${ANSI.yellow}${audit.notInEnvTs.length}${ANSI.reset} key(s) present but not validated by env.ts (deprecated or unmanaged)`
    );
    const keyWidth = Math.max(
      "Key".length,
      ...audit.notInEnvTs.map((r) => r.key.length)
    );
    console.log(
      `  ${pad(`${ANSI.dim}Key${ANSI.reset}`, keyWidth)}  ${ANSI.dim}Seen on${ANSI.reset}`
    );
    for (const r of audit.notInEnvTs) {
      console.log(`  ${pad(r.key, keyWidth)}  ${ANSI.dim}${r.sources.join(", ")}${ANSI.reset}`);
    }
  }
  if (audit.missingFromExample.length > 0) {
    console.log("");
    console.log(
      `  ${ANSI.yellow}${audit.missingFromExample.length}${ANSI.reset} key(s) validated by env.ts but missing from .env.example`
    );
    for (const k of audit.missingFromExample) {
      console.log(`    ${k}`);
    }
  }
  if (audit.missingFromEnvTs.length > 0) {
    console.log("");
    console.log(
      `  ${ANSI.yellow}${audit.missingFromEnvTs.length}${ANSI.reset} key(s) in .env.example but not validated by env.ts (deprecated?)`
    );
    for (const k of audit.missingFromEnvTs) {
      console.log(`    ${k}`);
    }
  }
}
